            ...
            <div class="bg-zinc-900 border border-zinc-800/60 rounded-2xl overflow-hidden group">
                <div class="flex flex-col md:flex-row">
                    <div class="md:w-2/5 relative h-64 md:h-auto overflow-hidden">
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-zinc-900 to-transparent md:bg-gradient-to-r">
                        </div>
                    </div>
            ...
            <div class="mt-8">
                        ...
                        @foreach ($comments as $comment)
                            <div class="mb-6 bg-zinc-900 border border-zinc-800 rounded-xl p-4">
                                <div class="flex items-start gap-3">
                                    <div
                                        class="w-10 h-10 bg-cyan-500/20 rounded-full flex items-center justify-center flex-shrink-0">
                                        <span
                                            class="text-cyan-400 font-bold text-sm">{{ strtoupper(substr($comment->user->name, 0, 1)) }}</span>
                                    </div>
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2 mb-2">
                                            <span class="text-white font-medium">{{ $comment->user->name }}</span>
                                            <span
                                                class="text-zinc-500 text-sm">{{ $comment->created_at->diffForHumans() }}</span>
                                        </div>
                ...
                @else
                    <div class="mt-6 bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-center">
                        ...
                        <a href="{{ route('login') }}"
                            class="bg-cyan-500 hover:bg-cyan-400 text-black px-6 py-2 rounded-lg font-bold text-sm transition-all">
                            Login to Comment
                        </a>
                    </div>
                @endif
        ...
        <div class="lg:col-span-5">
            <div
                ...
                <div class="mt-8 space-y-4">
                    <div class="space-y-2">
                        <label class="text-[10px] uppercase tracking-widest text-zinc-500 font-bold">Cardholder
                            Name</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label
                                class="text-[10px] uppercase tracking-widest text-zinc-500 font-bold">Expiration</label>
                ...
                <div class="flex justify-center gap-4 mt-10">
                    @if(auth()->check())
                        @if($isPurchased)
                            <a href="{{ route('models.download', $threed->id) }}"
                                class="bg-gradient-to-r from-green-400 to-green-500 hover:from-green-300 hover:to-green-400 text-black px-8 py-3 rounded-xl font-bold text-sm transition-all active:scale-95 flex items-center gap-2">
                        ...
                        @elseif($isInCart)
                            <a href="{{ route('cart') }}"
                                class="bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-300 hover:to-yellow-400 text-black px-8 py-3 rounded-xl font-bold text-sm transition-all active:scale-95 flex items-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                    stroke="currentColor" class="size-5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        @else
                            <form action="{{ route('cart.add', $threed->id) }}" method="POST" class="flex-1">
                                @csrf
                                <button type="submit"
                                    class="bg-gradient-to-r from-cyan-400 to-cyan-500 hover:from-cyan-300 hover:to-cyan-400 text-black px-8 py-3 rounded-xl font-bold text-sm transition-all active:scale-95 flex items-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                        stroke="currentColor" class="size-5">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                    ...
                    @else
                        <a href="{{ route('login') }}"
                            class="bg-gradient-to-r from-cyan-400 to-cyan-500 hover:from-cyan-300 hover:to-cyan-400 text-black px-8 py-3 rounded-xl font-bold text-sm transition-all active:scale-95">
                            Login to Purchase
                        </a>
                    @endif
                </div>
